...
class AppMusic {
  ...
  start() {
    ...
    if (!this.ctx || this.ctx.state === "closed") {
      ...
      // One noise buffer shared by all snares and hats — no repeated allocation
    ...
    // Poll every 2 seconds — far less CPU than 100ms
  ...
  private _schedule() {
    ...
    while (this.nextBarTime < horizon) {
      scheduleBar(this.ctx, this.nextBarTime, this.masterGain, this.barIndex, this.noiseBuf);
